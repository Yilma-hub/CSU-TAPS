    ...
    const o = { animation: false, color: PALETTE,
      ...
      series: [{ type: 'custom', coordinateSystem: 'cartesian2d', data, encode: { x: -1, y: -1, tooltip: 1 },
        renderItem: (params, api) => {
          ...
          if (/^\d{1,2}[A-L]$/.test(s.name || '')) { const c = api.coord([s.cx, s.cy]); kids.push({ type: 'text', x: c[0], y: c[1], style: { text: s.name, fill: '#1F2A22', fontSize: 11, fontWeight: 600, align: 'center', verticalAlign: 'middle' }, silent: true }); }
