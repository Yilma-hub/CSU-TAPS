...
(() => {
  ...
  // look settings kept for this browser: chart colors, map color scheme, satellite imagery
  const LOOK = (() => { try { return Object.assign({ palette: 'CSU-TAPS report', ramp: 'Red-Yellow-Green', reverse: false, imagery: false, opacity: 0.75 }, JSON.parse(localStorage.getItem('taps_look') || '{}')); }
    catch (e) { return { palette: 'CSU-TAPS report', ramp: 'Red-Yellow-Green', reverse: false, imagery: false, opacity: 0.75 }; } })();
  const saveLook = () => { try { localStorage.setItem('taps_look', JSON.stringify(LOOK)); } catch (e) {} };
  Charts.setPalette(LOOK.palette);
  const geoCache = {};
  ...
  async function pageMap() {
    ...
    $('pageNote').textContent = 'The field layout for the season. Each plot is colored by the value chosen on the left, from low to high as shown in the legend under the map; grey has no value. Click a plot for its details. The color scheme and satellite imagery are under Map look.';
    ...
    L.append(el('h3', { text: 'Season' }), seg(years.filter(y => manifest.datasets.find(d => d.id === 'map_' + y)), M.year, y => { M.year = y; S.season = y; pageMap(); }),
      ...
      el('h3', { text: 'Map look' }),
      sel('Background', [['plain', 'Plain'], ['imagery', 'Satellite imagery (Esri World Imagery)']], LOOK.imagery ? 'imagery' : 'plain', v => { LOOK.imagery = v === 'imagery'; saveLook(); pageMap(); }),
      LOOK.imagery ? el('label', {}, 'Plot color opacity', el('input', { type: 'range', min: 0.2, max: 1, step: 0.05, value: LOOK.opacity, on: { input: e => { LOOK.opacity = +e.target.value; saveLook(); drawMap(); } } })) : null,
      sel('Color scheme (numbers)', Object.keys(Charts.RAMPS).map(k => [k, k]), LOOK.ramp, v => { LOOK.ramp = v; saveLook(); drawMap(); }),
      el('label', { className: 'toggle' }, el('input', { type: 'checkbox', checked: LOOK.reverse, on: { change: e => { LOOK.reverse = e.target.checked; saveLook(); drawMap(); } } }), 'Reverse the colors'),
      sel('Colors (categories)', Object.keys(Charts.PALETTES).map(k => [k, k]), LOOK.palette, v => { LOOK.palette = v; Charts.setPalette(v); saveLook(); drawMap(); }));
    ...
    async function drawMap() {
      ...
      let tileTimer = 0;
      const res = Charts.plotMap(ds, idx, st, geoCache[M.year], M.year, { imagery: LOOK.imagery, opacity: LOOK.opacity, ramp: LOOK.ramp, reverse: LOOK.reverse,
        pxWidth: $('chart').clientWidth, onTile: () => { clearTimeout(tileTimer); tileTimer = setTimeout(() => { if (S.mode === 'map' && lastMapOption) chart.setOption(lastMapOption, true); }, 120); } });
      if (!res) { $('chartMsg').textContent = 'Nothing to draw.'; return; }
      fitMapGrid(res.option);
      lastMapOption = res.option;
  ...
  function renderChartPanel() {
    ...
    st.right = st.right || []; st.extra = st.extra || [];
    const canRight = !['pie', 'box', 'hbar', 'hstacked'].includes(st.type);
    const ybox = el('div', { className: 'checks' }, nums.map(c => el('div', { className: 'yrow' },
      el('label', {}, el('input', { type: 'checkbox', checked: st.ys.includes(c.key), on: { change: e => {
        st.ys = e.target.checked ? [...st.ys, c.key] : st.ys.filter(k => k !== c.key); renderChartPanel(); drawChart(); } } }), Data.label(c)),
      canRight && st.ys.includes(c.key) ? el('label', { className: 'rightax', title: 'Draw this value against a second axis on the right' },
        el('input', { type: 'checkbox', checked: st.right.includes(c.key), on: { change: e => { st.right = e.target.checked ? [...st.right, c.key] : st.right.filter(k => k !== c.key); drawChart(); } } }), 'Right axis') : null)));
    ...
    p.append(sel('Colors', Object.keys(Charts.PALETTES).map(k => [k, k]), LOOK.palette, v => { LOOK.palette = v; Charts.setPalette(v); saveLook(); drawChart(); }));
    if (!['pie', 'box', 'hbar', 'hstacked'].includes(st.type)) p.append(extraLinesBox(ds, st));
    appendFilters(p, ds, st.filters, st.range, drawChart);
  }

  // "Add a line": another value from the same data, picked by one column (for example Variable = Max temperature), drawn on the same figure
  function extraLinesBox(ds, st) {
    const box = el('div', { className: 'filter' }, el('div', { className: 'filter-head', text: 'More lines on the figure' }));
    st.extra.forEach((e, n) => box.append(el('div', { className: 'xline' },
      el('span', { text: `${e.label}${e.right ? ' (right axis)' : ''}` }),
      el('a', { text: 'Remove', on: { click: () => { st.extra.splice(n, 1); renderChartPanel(); drawChart(); } } }))));
    const cats = ds.columns.filter(c => c.type === 'cat' && c.key !== 'Year' && Data.uniques(ds, c.key).length <= 200);
    const nums = ds.columns.filter(c => c.type === 'num');
    if (!nums.length) return box;
    const f = { col: (cats.find(c => c.key === 'Variable') || cats.find(c => c.key === 'Index') || {}).key || '', val: '', y: (nums.find(c => c.key === 'Value') || nums[0]).key, right: true, kind: 'line', cum: false };
    const seasonRows = () => Data.filter(ds, st.season && st.season !== 'both' ? { Year: new Set([st.season]) } : {}, null);
    const valSel = el('select');
    const fillVals = () => { valSel.textContent = ''; const vs = f.col ? Data.uniques(ds, f.col, seasonRows()) : []; vs.forEach(v => valSel.append(el('option', { value: String(v), text: String(v) }))); f.val = vs.length ? String(vs[0]) : ''; };
    valSel.addEventListener('change', e => { f.val = e.target.value; });
    const valLab = el('label', {}, 'is', valSel); valLab.hidden = !f.col;
    const form = el('div', { className: 'xform' },
      cats.length ? sel('Rows where', [['', 'All rows (no pick)']].concat(cats.map(c => [c.key, Data.label(c)])), f.col, v => { f.col = v; fillVals(); valLab.hidden = !v; }) : null,
      valLab,
      sel('Value to draw', nums.map(c => [c.key, Data.label(c)]), f.y, v => { f.y = v; }),
      sel('Draw as', [['line', 'Line'], ['bar', 'Bars']], f.kind, v => { f.kind = v; }),
      sel('Axis', [['right', 'Right (second) axis'], ['left', 'Left axis']], 'right', v => { f.right = v === 'right'; }),
      el('label', { className: 'toggle' }, el('input', { type: 'checkbox', on: { change: e => { f.cum = e.target.checked; } } }), 'Running total'),
      el('button', { className: 'btn primary', text: 'Add line', on: { click: () => {
        const c = Data.col(ds, f.col), val = f.col ? Data.uniques(ds, f.col).find(v => String(v) === f.val) : null;
        const label = (f.col ? `${f.val}` : Data.label(Data.col(ds, f.y))) + (f.col && f.y !== 'Value' ? ` (${Data.label(Data.col(ds, f.y))})` : '');
        st.extra.push({ col: f.col || null, val, y: f.y, right: f.right, kind: f.kind, cum: f.cum, label: label + (f.cum ? ', running total' : '') });
        renderChartPanel(); drawChart(); } } }));
    fillVals();
    box.append(el('div', { className: 'hint', text: 'Add another value to this figure, for example rain on the left and Max temperature on the right. It uses the same X axis, season and date range.' }), form);
    return box;
  }
  function extrasFor(ds, st, af) {
    return (st.extra || []).map(e => {
      const f = Object.assign({}, af); if (e.col) { delete f[e.col]; f[e.col] = new Set([e.val]); }
      return Object.assign({}, e, { idx: Data.filter(ds, f, st.range) });
    });
  }
  ...
  $('pngBtn').addEventListener('click', () => {
    ...
    let url;
    try { url = chart.getDataURL({ type: 'png', pixelRatio: 2, backgroundColor: '#fff' }); }
    catch (e) { alert('The browser blocked saving this map with the satellite imagery. Switch Background to Plain and save again.'); return; }
