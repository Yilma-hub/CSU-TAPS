...
(() => {
  ...
  async function pageMap() {
    ...
    L.append(el('h3', { text: 'Season' }), seg([2024, 2025].filter(y => years.includes(y)), M.year, y => { M.year = y; pageMap(); }),
  ...
  function initState(ds, preset) {
    ...
    const years = ds.data.Year ? Data.uniques(ds, 'Year') : [];
  ...
  function renderChartPanel() {
    ...
    if (st.season !== null) {
      const years = Data.uniques(ds, 'Year');
      p.append(el('h3', { text: 'Season' }), seg(years.concat(['both']), st.season, y => { st.season = y; renderChartPanel(); drawChart(); }, years.map(String).concat(['Both'])));
    }
  ...
  function drawChart() {
    ...
    const idx = Data.filter(ds, activeFilters(st), st.range);
  ...
  async function pageTables(id) {
    ...
    L.append(el('h3', { text: 'Tables' }), sel('Topic', [['', 'All topics']].concat(groups.map(g => [g, g])), T.topic, t => { T.topic = t; fill(); }), lst);
    fill();
    ...
    if (st.season !== null) {
      const years = Data.uniques(ds, 'Year');
      const segBox = el('div');
      const drawSeg = () => { segBox.textContent = ''; segBox.append(seg(years.concat(['both']), st.season, y => { st.season = y; T.page = 0; drawSeg(); drawTable(); }, years.map(String).concat(['Both']))); };
      drawSeg(); P.append(el('h3', { text: 'Season' }), segBox);
    }
