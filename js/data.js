...
const Data = (() => {
  ...
  function plotKey(v) { const m = /^(\d{1,2})([A-I])$/.exec(String(v)); return m ? [+m[1], m[2]] : null; }
  ...
  async function downloadXLSX(name, sheets) {
    ...
    for (const [sn, rows] of sheets) XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(rows), sn.slice(0, 31));
    XLSX.writeFile(wb, name, { compression: true });
  }

  return { getManifest, load, col, label, fmt, uniques, filter, group, boxStats, rowsFor, downloadCSV, downloadXLSX, tsToDate, sortVals, AGG };
})();
